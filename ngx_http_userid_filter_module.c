...
static uint32_t muid_crc(uint32_t a, uint32_t b, uint32_t c);

static const uint8_t key[] = {0x13,0x23,0xae,0x9e,0x12,0x52,0xaf,0x83,0xf7,0xe1,0x21,0x77,0x21,0x33,0xcd,0xe2};
...
static void rc4_init(struct rc4_state *const state,
...
static ngx_int_t ngx_http_userid_valid(const uint32_t* uid_got);
    ...
    ngx_http_userid_conf_t *conf)
{
    ...
    /*
     * here we change the format of uid.
     * uid[0] = crc, crc = muid_crc(uid[1], uid[2], uid[3]);
     * uid[1] = time now.
     * uid[2] = random int();
     * uid[3] = ipnum
     */

    /* here we add muid check code.
    */
    if (ctx->uid_got[3] != 0 && NGX_OK == ngx_http_userid_valid(ctx->uid_got)) {
    ...
    switch (c->sockaddr->sa_family) {
        ...
        default: /* AF_INET */
            ...
            muid_val[2] = sin->sin_addr.s_addr;
            break;
    }

    muid_val[1] = (uint32_t) ngx_time();
    muid_val[3] = ((uint32_t)rand()) | 0x80000000;
    muid_val[0] = muid_crc(muid_val[1], muid_val[2], muid_val[3]);
    muid_val[1] ^= muid_val[0];
    muid_val[2] ^= muid_val[0];

    ctx->uid_set[0] = htonl(muid_val[0]);
    ctx->uid_set[1] = htonl(muid_val[1]);
    ctx->uid_set[2] = htonl(muid_val[2]);
    ctx->uid_set[3] = htonl(muid_val[3]);

    rc4_init(&stat, key, sizeof(key));
    rc4_crypt(&stat, (const uint8_t*)(&ctx->uid_set[1]), (uint8_t*)(&ctx->uid_set[1]), 8);
...
/*already base64 decode.*/
static ngx_int_t ngx_http_userid_valid(const uint32_t* uid_got)
{
    uint32_t			muid_val[4];
    struct rc4_state	stat;

    memcpy(muid_val, uid_got, sizeof(muid_val));

    rc4_init(&stat, key, sizeof(key));
    rc4_crypt(&stat, (const uint8_t*)(&uid_got[1]), (uint8_t*)(&muid_val[1]), 8);

    muid_val[1] ^= muid_val[0];
    muid_val[2] ^= muid_val[0];

    if (ntohl(muid_val[0]) == muid_crc(ntohl(muid_val[1]), ntohl(muid_val[2]), ntohl(muid_val[3])))
    {
        return NGX_OK;
    }
